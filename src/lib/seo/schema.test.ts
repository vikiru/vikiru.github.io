...
const visibleProjects = projectData.projects.filter((project) => !project.hidden);
...
describe("JSON-LD structured data", () => {
  ...
  it("every visible project has a graph with a BreadcrumbList", () => {
    for (const project of visibleProjects) {
      ...
      if (!graph) throw new Error(`missing graph for ${project.slug}`);
      const types = nodeTypes(graph);
      assert.ok(types.includes("BreadcrumbList"), `${project.slug} missing BreadcrumbList`);
    }
  });
  ...
  it("project breadcrumbs are sequential Home > Projects > name", () => {
    for (const project of visibleProjects) {
      ...
      if (!breadcrumb) throw new Error(`${project.slug} missing BreadcrumbList`);
  ...
  it("courses graph contains a BreadcrumbList", () => {
    ...
    const breadcrumb = breadcrumbOf(graph);
    if (!breadcrumb) throw new Error("courses graph missing BreadcrumbList");
    assert.deepEqual(
  ...
  it("all graphs serialize to valid JSON with non-empty graphs", () => {
    const graphs = [homeGraphSchema, coursesGraphSchema as Graph, ...projectSchemaMap.values()];
